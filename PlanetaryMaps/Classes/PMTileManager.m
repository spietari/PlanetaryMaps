...
-(void)addToTopOfQueue:(PMTile*)object
{
    @synchronized (self.downloadStack)
    {
        [self.downloadStack insertObject:object atIndex:0];
        for (NSInteger i = self.downloadStack.count - 1; i >= self.planetaryView.maxTilesInDownloadQueue; i--)
        {
...
-(void)removeQueued:(PMTile*)object
{
    @synchronized (self.downloadStack)
    {
        PMTile* itemToRemove = nil;
        for (PMTile* tile in self.downloadStack)
        {
            if ([tile isEqualToTile:object])
            {
...
-(void)receivedData:(NSData*)data forTile:(PMTile*)tile
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (NSInteger i = self.tileCache.count - 1; i >= self.planetaryView.cacheSize; i--)
            {
...
//        }
    ...
    for (NSInteger i = tiles.count - 1; i >= self.planetaryView.maxTilesInDownloadQueue; i--)
    {
        [tiles removeObjectAtIndex:i];
    }
}
...
-(void)optimizeDisplayOrderForTiles:(NSMutableArray*)tiles
{
    ...
    while (tiles.count > self.planetaryView.maxTiles)
    {
